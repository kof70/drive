...
export const FileRenderer: React.FC<ElementRendererProps> = (props) => {
  const { element } = props;
  ...
  const handleDownload = async (event: React.MouseEvent) => {
    ...
    try {
      await fileUploadService.downloadFile(fileRef.storedPath, fileRef.filename);
    } catch (error) {
  ...
  const handlePreview = (event: React.MouseEvent) => {
    ...
    if (fileRef.mimeType.startsWith('image/')) {
      ...
      window.open(`${apiUrl}/api/files/download/${fileRef.storedPath}`, '_blank');
    } else {
