...
import { ElementRendererProps, ElementRendererComponent } from './BaseElementRenderer';
...
const rendererRegistry: Record<CanvasElement['type'], ElementRendererComponent> = {
  note: NoteRenderer,
...
export interface ElementRendererFactoryProps extends Omit<ElementRendererProps, 'element'> {
  element: CanvasElement;
}

export const ElementRendererFactory: React.FC<ElementRendererFactoryProps> = (props) => {
  const { element } = props;
...
export const useElementRenderer = () => {
  const registerRenderer = (type: CanvasElement['type'], renderer: ElementRendererComponent) => {
    rendererRegistry[type] = renderer;
  };

  const getRenderer = (type: CanvasElement['type']): ElementRendererComponent | undefined => {
