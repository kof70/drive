...
export const CanvasElements: React.FC<CanvasElementsProps> = ({ viewport }) => {
  const { elements, selectedElementIds, selectElement, moveElement } = useCanvasStore();
  const { emit } = useWebSocketContext();

  // Gérer le déplacement d'éléments
  const handleElementMove = (element: CanvasElement, newPosition: { x: number; y: number }) => {
  ...
  const handleContextMenu = (element: CanvasElement, event: React.MouseEvent) => {
    ...
    <div className="absolute inset-0">
      {elements.map((element) => (
        <ElementRendererFactory
          ...
          isDragging={dragState.isDragging && dragState.draggedElement?.id === element.id}
