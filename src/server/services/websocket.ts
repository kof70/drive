...
import { UserSession, CursorPosition, CanvasElement, ClipboardData } from '../../shared/types';
...
export class WebSocketService {
  ...
  private loadCanvasStateFromDatabase(): void {
    ...
    logger.info(`💾 ${elements.length} éléments chargés depuis la base de données`);
  }
  ...
  private setupEventHandlers(): void {
    this.io.on('connection', (socket) => {
      ...
      const userSession: UserSession = {
        id: socket.id,
        deviceName: this.extractDeviceName(socket.handshake.headers['user-agent'] || ''),
      ...
      logger.info(`📊 État du canvas synchronisé avec ${socket.id}: ${currentCanvasState.length} éléments`);

      // Gestionnaire pour les mises à jour du canvas
      socket.on('canvas-update', (data: CanvasElement) => {
        logger.debug(`📝 Mise à jour canvas de ${socket.id}: ${data.type} ${data.id}`);
      ...
      socket.on('canvas-element-add', (data: CanvasElement) => {
        logger.debug(`➕ Ajout élément canvas de ${socket.id}: ${data.type} ${data.id}`);
      ...
      socket.on('canvas-element-remove', (elementId: string) => {
        logger.debug(`➖ Suppression élément canvas de ${socket.id}: ${elementId}`);
      ...
      socket.on('request-canvas-state', () => {
        const currentCanvasState = Array.from(this.canvasState.values());
        socket.emit('canvas-state-sync', currentCanvasState);
        logger.debug(`📊 État du canvas envoyé à ${socket.id}: ${currentCanvasState.length} éléments`);
      });
  ...
  public broadcastToOthers(excludeSocketId: string, event: string, data: any): void {
